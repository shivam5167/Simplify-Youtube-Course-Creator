...
void main() {
    ...
    fragColor.rgb =height * rampColor;
    fragColor.a = max(0.0,height)*0.6;
}
`;
...
export default function Aurora(props: AuroraProps) {
  const { colorStops = ["#00d8ff", "#7cff67", "#00d8ff"], amplitude = 1 } =
    props;
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      alpha: true,
      premultipliedAlpha: true, // Enable premultiplied alpha
    ...
    const colorStopsArray = colorStops.map((hex) => {
      ...
      if (!r || !g || !b || !a) return [0, 0, 0] as [number, number, number];
      // Return only RGB, we'll handle alpha separately
      return [r * a, g * a, b * a] as [number, number, number];
  ...
  return (
    <div
      ref={ctnDom}
      className="pointer-events-none absolute bottom-0 left-0 z-[-1] size-full rotate-180 bg-transparent"
    />
  );
}
